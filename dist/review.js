...
import { Commenter, COMMENT_REPLY_TAG, RAW_SUMMARY_END_TAG, RAW_SUMMARY_START_TAG, SHORT_SUMMARY_END_TAG, SHORT_SUMMARY_START_TAG, SUMMARIZE_TAG } from './commenter';
...
export const codeReview = async (lightBot, heavyBot, options, prompts) => {
    ...
    if (highestReviewedCommitId === '' ||
        ...
        info(`Will review from the base commit: ${context.payload.pull_request.base.sha}`);
...
Files that changed in the full PR diff from base to head ${context.payload.pull_request.head.sha} commits.
</details>
...
`;
    ...
    const doSummary = async (filename, fileContent, fileDiff) => {
        ...
        try {
            ...
            warning(`summarize: error from openai: ${e}`);
    ...
    if (options.disableReleaseNotes === false) {
        ...
        if (releaseNotesResponse === '') {
            ...
            try {
                ...
                warning(`release notes: error from github: ${e.message}`);
            }
        }
    }
